...
std::string Proxy::getDebugInfo() {
    ...
    ss << "P: " << getPing() << " RP: " << getRealPing() << " In: " << m_packetsRecived << " (" << m_bytesRecived
            << ")  Out: " << m_packetsSent << " (" << m_bytesSent << ") Conns: " << m_connections << " Sess: " <<
            m_sessions << " R: " << m_resolvedIp;
    return ss.str();
}
...
void Proxy::onPing(uint32_t packetId) {
    ...
    m_waitingForPing = false;
    m_ping = (uint32_t) std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::high_resolution_clock::now() - m_lastPingSent).count();
}
...
void Proxy::readHeader() {
    boost::asio::async_read(m_socket, boost::asio::buffer(m_buffer, 2),
                            std::bind(&Proxy::onHeader, shared_from_this(), std::placeholders::_1,
                                      std::placeholders::_2));
}
...
void Proxy::onHeader(const boost::system::error_code &ec, std::size_t bytes_transferred) {
    ...
    boost::asio::async_read(m_socket, boost::asio::buffer(m_buffer, packetSize),
                            std::bind(&Proxy::onPacket, shared_from_this(), std::placeholders::_1,
                                      std::placeholders::_2));
}
...
void Proxy::send(const ProxyPacketPtr &packet) {
    ...
    if (sendNow) {
        boost::asio::async_write(m_socket, boost::asio::buffer(packet->data(), packet->size()),
                                 std::bind(&Proxy::onSent, shared_from_this(), std::placeholders::_1,
                                           std::placeholders::_2));
    }
}
...
void Proxy::onSent(const boost::system::error_code &ec, std::size_t bytes_transferred) {
    ...
    if (!m_sendQueue.empty()) {
        boost::asio::async_write(
            m_socket, boost::asio::buffer(m_sendQueue.front()->data(), m_sendQueue.front()->size()),
...
void Session::check(const boost::system::error_code &ec) {
    ...
    uint32_t lastPacket = (uint32_t) std::chrono::duration_cast<std::chrono::milliseconds>(
...
void Session::selectProxies() {
    ...
    if (candidate_proxy) {
        ...
        bool disconnectWorst = worst_ping && worst_ping != best_ping && worst_ping->getPing() > candidate_proxy->
                               getPing() + 20;
...
void Session::onProxyPacket(uint32_t packetId, uint32_t lastRecivedPacketId, const ProxyPacketPtr &packet) {
    ...
    boost::asio::async_write(m_socket, boost::asio::buffer(packet->data(), packet->size()),
                             std::bind(&Session::onSent, shared_from_this(), std::placeholders::_1,
                                       std::placeholders::_2));
}
...
void Session::onHeader(const boost::system::error_code &ec, std::size_t bytes_transferred) {
    ...
    boost::asio::async_read(m_socket, boost::asio::buffer(m_buffer + 2, packetSize),
                            std::bind(&Session::onBody, shared_from_this(), std::placeholders::_1,
                                      std::placeholders::_2));
}
...
void Session::onSent(const boost::system::error_code &ec, std::size_t bytes_transferred) {
    ...
    if (!m_sendQueue.empty() && m_sendQueue.begin()->first == m_inputPacketId) {
        boost::asio::async_write(
            m_socket, boost::asio::buffer(m_sendQueue.begin()->second->data(), m_sendQueue.begin()->second->size()),
