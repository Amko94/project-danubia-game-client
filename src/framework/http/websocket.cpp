...
void WebsocketSession::start() {
    ...
    if (m_url.find("wss") == 0 || m_url.find("WSS") == 0) {
        ...
        m_ssl = std::make_shared<boost::beast::websocket::stream<boost::beast::ssl_stream<boost::beast::tcp_stream> > >(
            m_service, *m_context);
    ...
    m_resolver.async_resolve(m_domain, std::to_string(m_port),
                             std::bind(&WebsocketSession::on_resolve, shared_from_this(), std::placeholders::_1,
                                       std::placeholders::_2));
}
...
void WebsocketSession::send(std::string data) {
    ...
    if (sendNow) {
        if (m_ssl) {
            m_ssl->async_write(boost::asio::buffer(m_sendQueue.front(), m_sendQueue.front().size()),
                               std::bind(&WebsocketSession::on_send, shared_from_this(), std::placeholders::_1));
        } else {
            m_socket->async_write(boost::asio::buffer(m_sendQueue.front(), m_sendQueue.front().size()),
                                  std::bind(&WebsocketSession::on_send, shared_from_this(), std::placeholders::_1));
        }
    }
}
...
void WebsocketSession::on_resolve(const boost::system::error_code &ec,
                                  boost::asio::ip::tcp::resolver::results_type iterator) {
    if (ec)
        return onError("resolve error", ec.message());
    boost::asio::ip::tcp::endpoint endpoint(iterator.begin()->endpoint().address(), m_port);
    if (m_ssl) {
        boost::beast::get_lowest_layer(*m_ssl).async_connect(
            endpoint, std::bind(&WebsocketSession::on_connect, shared_from_this(), std::placeholders::_1));
    } else {
...
void WebsocketSession::on_connect(const boost::system::error_code &ec) {
    ...
    if (m_url.find("wss") == 0 || m_url.find("WSS") == 0) {
        ...
        auto self(shared_from_this());
        m_ssl->next_layer().async_handshake(boost::asio::ssl::stream_base::client,
                                            [&, self](const boost::system::error_code &ec) {
                                                if (ec)
                                                    return onError("WSS handshake error", ec.message());

                                                auto parsedUrl = parseURI(m_url);
                                                m_ssl->async_handshake(m_domain, parsedUrl.query,
                                                                       std::bind(&WebsocketSession::on_handshake,
    ...
    auto parsedUrl = parseURI(m_url);
    m_socket->async_handshake(m_domain, parsedUrl.query,
                              std::bind(&WebsocketSession::on_handshake, shared_from_this(), std::placeholders::_1));
}
...
void WebsocketSession::on_handshake(const boost::system::error_code &ec) {
    ...
    if (m_ssl) {
        m_ssl->async_read(m_streambuf, std::bind(&WebsocketSession::on_read, shared_from_this(), std::placeholders::_1,
                                                 std::placeholders::_2));
        ...
        m_socket->async_read(m_streambuf,
                             std::bind(&WebsocketSession::on_read, shared_from_this(), std::placeholders::_1,
                                       std::placeholders::_2));
        if (!m_sendQueue.empty()) {
            m_socket->async_write(boost::asio::buffer(m_sendQueue.front(), m_sendQueue.front().size()),
                                  std::bind(&WebsocketSession::on_send, shared_from_this(), std::placeholders::_1));
