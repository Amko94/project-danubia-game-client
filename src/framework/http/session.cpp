...
void HttpSession::start() {
    ...
    m_resolver.async_resolve(m_domain, std::to_string(m_port),
                             std::bind(&HttpSession::on_resolve, shared_from_this(), std::placeholders::_1,
                                       std::placeholders::_2));
}

void HttpSession::on_resolve(const boost::system::error_code &ec,
                             boost::asio::ip::tcp::resolver::results_type iterator) {
    if (ec)
        return onError("resolve error", ec.message());
    auto endpoint = *iterator.begin();
    ...
    if (m_url.find("https") == 0 || m_url.find("HTTPS") == 0) {
        ...
        m_ssl->async_handshake(boost::asio::ssl::stream_base::client, [&, self](const boost::system::error_code &ec) {
            ...
            boost::beast::http::async_write(*m_ssl, m_request,
                                            std::bind(&HttpSession::on_request_sent, shared_from_this(),
                                                      std::placeholders::_1));
        });
    } else {
...
void HttpSession::on_read_header(const boost::system::error_code &ec, size_t bytes_transferred) {
    ...
    if (!location.empty()) {
        auto session = std::make_shared<HttpSession>(m_service, std::string(location), m_agent, m_timeout, m_isJson,
                                                     m_result, m_callback);
        session->start();
        return close();
    }
...
void HttpSession::on_read(const boost::system::error_code &ec, size_t bytes_transferred) {
    ...
    if (ec && ec != boost::beast::http::error::end_of_stream)
        ...
        if (!m_result->finished) {
            ...
            for (auto b: buffers) {
                m_result->response.insert(m_result->response.end(), static_cast<const uint8_t *>(b.data()),
                                          static_cast<const uint8_t *>(b.data()) + b.size());
            }
            m_callback(m_result);
        }
        return close();
    ...
    if (m_result->size > 0) {
        //m_callback
        int new_progress = (int) std::min<int64_t>(
            100ll, (100ll * (int64_t) m_response.get().payload_size().get_value_or(0)) / (int64_t) m_result->size);
